...
def pulse_packet(t, n, sigma,repeats):
	...
	for repeat in xrange(repeats):
		times = sigma*(np.random.randn(n))+2*(repeat+1)*t #Straight Numpy doesn't recognize units, more general way to do this?
		times.sort()
		neuron = range(len(times))
		shuffle(neuron)

		neuron_list.extend(neuron)
		times_list.extend(times)

	return zip(neuron_list, times_list) # returns a list of pairs (i,t)



def minimal_example(N):
	...
	equations = Equations('''
		...
		du/dt = (N*v0-u)/taum 									 : volt 
		''')

	def myreset(P,spikes,groupsize=100):
		for i,group in enumerate(Pgp):
			gspikes = np.mod(spikes[(i*groupsize):(i+1)*groupsize],groupsize)
			group.V[gspikes] = Vr #Normal reset
			group.V += g_gap * beta * len(gspikes) #effect of spikes
			group.u -= delta * len(gspikes)

	# Neuron groups
	P = NeuronGroup(N=N, model=equations,
				  threshold=Vt, reset=myreset)
...
#                  threshold=Vt,reset=Vr,refractory=1*ms)
	...
	sigma = 1*ms 
	repeats = 5
	Pinput = SpikeGeneratorGroup(n,pulse_packet(t,n,sigma,5))
